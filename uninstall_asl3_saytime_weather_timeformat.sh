...
#
# This removes the scripts, WeatherAPI config/cache, systemd files,
# crontab entries, downloaded recorded voice files, and old /tmp sound_files.zip copies.
#
# Usage:
...
confirm_uninstall() {
    ...
    echo
    echo "  Downloaded recorded voice files from:"
    echo "    $SOUNDS_DIR"
    echo
    echo "  Old temporary sound ZIP files:"
    echo "    /tmp/sound_files.zip"
    echo "    /tmp/sound_files*.zip"
    echo
    echo "CAUTION:"
    echo "  This removes .gsm files and common voice-file subdirectories from:"
    echo "    $SOUNDS_DIR"
    echo
    echo "  If other projects use those same voice files, they may need to be reinstalled."
    echo
...
remove_cache() {
    ...
    # Remove old downloaded voice ZIP copies from /tmp.
    find /tmp -maxdepth 1 -type f \( -name 'sound_files.zip' -o -name 'sound_files*.zip' \) -print -delete 2>/dev/null | \
...
remove_sound_files() {
    log "Removing downloaded recorded voice files."

    if [ ! -d "$SOUNDS_DIR" ]; then
        log "Sound directory not found, skipping $SOUNDS_DIR"
        return 0
    fi

    # Remove GSM/ULAW/WAV voice files commonly installed by the KD5FMU sound package.
    find "$SOUNDS_DIR" -type f \( -name '*.gsm' -o -name '*.ul' -o -name '*.ulaw' -o -name '*.wav' \) -print -delete 2>/dev/null | \
        while read -r removed; do
            log "Removed $removed"
        done

    # Remove common voice-file subdirectories if they are empty after file cleanup.
    find "$SOUNDS_DIR" -depth -type d \( -name 'digits' -o -name 'wx' -o -name 'weather' -o -name 'silence' -o -name 'dictate' \) -empty -print -delete 2>/dev/null | \
        while read -r removed_dir; do
            log "Removed empty directory $removed_dir"
        done

    log "Sound-file cleanup complete for $SOUNDS_DIR"
}
...
show_done_message() {
    ...
    echo "  find /tmp -maxdepth 1 -name 'sound_files*.zip' -print"
    echo "  find $SOUNDS_DIR -type f -name '*.gsm' | head"
    echo
    echo "73!"
    echo "======================================================================"
    echo
